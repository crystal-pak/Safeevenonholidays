...
import { getList } from "../api/hospitalApi";
  ...
  const [serverData, setServerData] = useState(initState)
    ...
    const districts = {
      ...
      "경기도": [
        ...
        "남양주시", "안양시", "화성시", "평택시", "시흥시", "파주시", 
        "김포시", "광명시", "군포시", "광주시", "이천시", "양주시", 
        "오산시", "구리시", "안성시", "포천시", "여주시", "동두천시", 
        "과천시", "가평군", "양평군", "연천군"
      ],
      "강원도": [
  ...
  return (
    <Container className="mt-4 mb-4 p-5" style={{ maxWidth: '700px' }}>
      ...
      <h1 className="header">병원 검색</h1>
        ...
        <select className="select" value={district} onChange={(e) => setDistrict(e.target.value)} disabled={!city}>
        ...
        <select className="select" value={departmentSearch} onChange={(e) => setDepartmentSearch(e.target.value)}>
            ...
            <Card className="text-center search-card">
              <Card.Body>
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                  <Card.Text className="search-card-distanc">
                  {item.distance ? `${item.distance.toFixed(2)} km` : "위치를 찾을 수 없습니다."}
                  </Card.Text>
